...
set AUTO443=--dpi-desync=fake,multisplit --dpi-desync-fake-tls="%~dp0fake\tls_clienthello_stepik_org.bin" --dpi-desync-split-seqovl=1 --dpi-desync-split-pos=1,midsld,endsld+1,sniext+2,host+1,endhost-3 --dpi-desync-fooling=md5sig --dpi-desync-autottl=1 --dpi-desync-repeats=3
...
set YTDISQ443=--dpi-desync=fake,udplen --dpi-desync-fake-quic="%~dp0fake\quic_initial_www_google_com.bin" --dpi-desync-udplen-increment=12 --dpi-desync-udplen-pattern=0xDEADBEEF --dpi-desync-cutoff=n2 --dpi-desync-autottl=1 --dpi-desync-repeats=6

set TW443=--dpi-desync=fake,multisplit --dpi-desync-fake-tls="%~dp0fake\tls_clienthello_rutube_ru.bin" --dpi-desync-split-seqovl=1 --dpi-desync-split-pos=1,midsld+1,sniext+3,host+2 --dpi-desync-fooling=md5sig --dpi-desync-autottl=1 --dpi-desync-repeats=4

set BLACKLIST443=--dpi-desync=fake,multisplit --dpi-desync-fake-tls="%~dp0fake\tls_clienthello_dzen_ru.bin" --dpi-desync-split-seqovl=574 --dpi-desync-split-seqovl-pattern="%~dp0fake\tls_clienthello_mail_google_com.bin" --dpi-desync-split-pos=sld,sld+7,midsld,midsld+9,sniext,sniext+25,host-3,host+11,endhost-14 --dpi-desync-fooling=md5sig,badseq --dpi-desync-autottl=1 --dpi-desync-repeats=6

set BLACKLIST80=--dpi-desync=fake,multidisorder --dpi-desync-split-pos=1,method+2,host+3 --dpi-desync-fooling=md5sig,datanoack
:: -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= ::
